...
Thing.prototype.define = function(){
    ...
    if (arguments.length === 0){
        ...
        // define(propertyDefinitionArray)
            ...
            throw new Error("Please pass a single or array of property definitions");
        }
...
Thing.prototype.definition = function(propertyName){
    if (!propertyName) throw new Error("please supply an propertyName");
...
function definitions(){
    var output = {};
    _.each(this._definitions, function(def, property){
...
Thing.prototype.set = function set(property, value){
    ...
    if (property){
        if (property instanceof Thing){
            var config = property;
            config.properties.forEach(function(propertyName){
                self.set(propertyName, config.get(propertyName));
            });
...
function getValid() {
    return this._errors.length === 0 && _.every(this.definitions, function(def, property){
        return def.valid;
    });
}
...
@type Array
*/
Object.defineProperty(Thing.prototype, "properties", { enumerable: true, get: getOptions });
function getOptions(){
...
/**
@method get
@param {String} property Option name
...
Thing.prototype.mixIn = function(config, groups){
    ...
    if (config instanceof Thing){
        config.properties.forEach(function(propertyName){
            if (groups){
                self.define(groups, config.definition(propertyName));
            } else {
                self.define(config.definition(propertyName));
            }
        });
    } else {
