...
function Config(){
                ...
                if (defaultValues.length > 0){
                    ...
                    _.each(_definitions, function(definition, optionName){
                        if (definition.defaultOption){
                            self.set(optionName, defaultValues.length == 1
                                ? defaultValues[0]
                                : defaultValues
                            );
                        }
    ...
    */
    ...
    function setValue(option, value){
        ...
        //     throw new Error(util.format("Invalid Type. Option: %s, value: %s", option, value));
        // }
        // if (typeof def.type === "string" && typeof value !== def.type.toLowerCase()){
        //     throw new Error(util.format("Invalid Type (%s). Option: %s, value: %s, def: %s", typeof value, option, value, JSON.stringify(def)));
        // } 
...
function getErrors(){
    ...
    this.options.forEach(function(option){
        output = output.concat({
            option: option,
            errors: self.definition(option).errors   
        });
    });
    return output;
}
